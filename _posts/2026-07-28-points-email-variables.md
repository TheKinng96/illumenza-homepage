...
<figure>
  <img src="/images/blog/points-email-variables/screen.webp"
       alt="メールテンプレートの編集画面。件名が「【{shopName}】チケットをご利用いただきました」、本文に {memberName}様、使用チケット: {ticketsSpent}枚、残りチケット: {remainingTickets}枚 と変数が差し込まれている。下部に「利用可能な変数」として4つの変数がクリックしてコピーできる形で並ぶ。">
  <figcaption>使える変数はテンプレートごとに違います。編集画面の下に並んでいるものがそのテンプレートで使える全部です。</figcaption>
</figure>
...
**取引の事実を述べている部分。** チケットが何枚使われた、いつ承認された、といった記述は、正確であることがすべてです。言い回しを工夫する場所ではありません。

**却下のテンプレートで、チケットが返還される旨の記述。** ここは削らないでください。会員がいちばん不安になる場面で、事実を伝えている一文です。
...
導入直後であれば、まず「チケット交換完了」と「ミッション完了」の2つを見ておけば十分です。この2つは、制度が動いていればいちばん多く送られます。
