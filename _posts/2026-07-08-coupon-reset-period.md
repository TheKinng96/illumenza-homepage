...
リセット期間は、**同じ人にどれくらいの頻度で参加してほしいか**を決める設定です。
...
売上アップのテンプレートは「短いリセット間隔で再スピンも可能」と説明されています。購入直前の後押しが目的なので、何度でも参加できるほうが機会が増える設計です。
...
上限に達した参加者には、専用のメッセージが出ます。「スピン回数が上限に達しました。時間をおいて再度お試しください」が既定の文面です。
