...
description: "通知パネルの文面には獲得チケットや特典名を差し込めます。会員と非会員で別の文面を出す設定と、プレビューで確認できること・できないことを説明します。"
...
<figure>
  <img src="/images/blog/points-nudge-copy/editor.webp"
       alt="チケット獲得のお知らせの編集画面。表示条件、詳細条件、デフォルト・会員・非会員のタブ、見出しとメッセージの入力欄、利用可能な変数として獲得チケット、ボタンテキスト、非会員向けの見出しとメッセージ、ボタンの表示先と色の設定が並び、右側にプレビューが表示されている。"
...
**具体的な数字が入ると、通知は急に自分ごとになります。** 「チケットを獲得しました」より「120枚を獲得しました」のほうが強い。変数を使わない理由はほとんどありません。

## 会員と非会員で文面を変えられます

チケット獲得のお知らせには、デフォルト・会員・非会員の3つのタブがあります。

非会員には、そもそもチケットが貯まっていません。同じ文面を出しても意味が通らないので、**「会員登録でチケットが貯まります」のような、登録を促す文面**が初期値になっています。
...
プレビューでは、変数にサンプル値が入った状態で表示されます。獲得チケットなら100、特典名なら「500円OFFクーポン」といった具合です。
