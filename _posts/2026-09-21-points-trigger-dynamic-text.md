...
これまで、パネルを開くボタンには1つの文言しか設定できませんでした。
...
<figure>
  ...
  <figcaption>上の行から順番に条件を確認し、最初に条件に当てはまった行の文言が表示されます。</figcaption>
</figure>
...
1と2の両方に当てはまるお客さまには、**1の「会員登録で300マイル」だけ**が表示されます。
...
と表示されていても、実際のショップで会員登録のマイルを設定していなければ、その行は表示されません。
...
スマホでも文言を表示する場合は、できるだけ短い言葉にするのがおすすめです。
