---
title: "会員に合わせてボタンの文言を変える — 表示テキストの条件設定"
date: 2026-09-21 09:00:00 +0900
tags: [ショップマイル, ロイヤルティ, カラーミーショップ]
description: "ボタンに表示する文言を、会員の状況に合わせて変えられるようになりました。上から順番に条件を確認し、最初に当てはまった文言を表示する仕組みと、条件・変数・文字数の設定方法を説明します。"
...
たとえば「特典を見る」と設定すると、会員にも、まだ会員登録していない方にも、同じ「特典を見る」と表示されます。

今回のアップデートで、**会員の状況に合わせて、ボタンの文言を変えられるようになりました。**
...
と設定した場合、1と2の両方に当てはまる会員には、**1の「会員登録で300マイル」だけ**が表示されます。
...
上のどの行にも当てはまらなかった会員には、この文言が表示されます。

基本テキストは削除したり、オフにしたりできません。

**どの会員にも、必ず何か1つ表示するための行**です。
...
この場合、**両方に当てはまる会員だけ**が対象になります。

条件を増やすほど、当てはまる会員は少なくなります。

なお、会員ステージは「シルバー以上」のように、ステージの高さで比べます。

まだ会員ステージがない会員は、「シルバー以上」などの条件には当てはまりません。

---

## 文言には、会員の情報を入れられます

文言の中に `{マイル残高}` のような文字を入れると、実際の画面では、会員の情報に置き換わります。
...
使えない情報を表示しようとして、会員に「0マイルが失効」のような表示が出ないようになっています。

会員向けの行では、`{マイル残高}` のように、会員から取得できる情報を使えます。
...
行を増やしすぎると、あとから「この会員には、なぜこの文言が出ているんだろう？」と分かりにくくなることがあります。
...
のように、**会員が見た瞬間に「自分に何ができるか」が分かる文言**にすると、この機能を活用しやすくなります。
