...
title: "Illumenza ブログ"
description: "カラーミーショップ運営者のための、チケット・ロイヤルティ施策とECマーケティングの実践ガイド。"
...
<article class="container mx-auto max-w-3xl">
  ...
  <aside class="mt-12 sm:mt-16 p-6 sm:p-8 bg-brand-light rounded-xl text-center">
    <h2 class="text-lg sm:text-xl font-bold text-gray-900 mb-3">チケット制度をノーコードで始めませんか</h2>
    <p class="text-sm sm:text-base text-gray-700 mb-6">
      ショップミッションは、カラーミーショップ向けのロイヤルティアプリです。チケット付与・会員クラス・友達紹介を、専門知識なしで設定できます。
    </p>
...
```markdown
---
title: "カラーミーショップでチケット制度を始める前に決めておく5つのこと"
date: 2026-07-30 09:00:00 +0900
tags: [チケット制度, ロイヤルティ, カラーミーショップ]
description: "チケット制度は「還元率を決めて終わり」ではありません。カラーミーショップで運用を始める前に決めておきたい5つの論点を、計算例つきで整理します。"
ogImage: /images/points.webp
---

チケット制度は、リピート購入を増やす施策のなかでも導入しやすいものです。一方で「とりあえず1%還元で始めた」あと、次のような迷いが出てくることがよくあります。

- 送料やクーポン利用分にもチケットを付けるべきか
...
なお、付与したチケットの全額がすぐ使われるわけではありません。未使用のまま失効する分（失効率）があるため、実際のコストは付与額より小さくなります。ただし失効率は運用実績が出るまで読めないので、**最初は失効を見込まずに** 成り立つ還元率から始めるのが安全です。

## 2. 何に対してチケットを付けるか

「購入金額の1%」と決めても、次の境界は自動的には決まりません。

- **送料** — 送料にチケットを付けると、送料無料ラインの直前でカゴ落ちしたお客様に、送料分の還元がつくことになります。送料を除外する運用が一般的です。
- **手数料** — 代引き手数料なども同様に除外を検討します。
- **クーポン利用後の金額か、利用前か** — 3,000円の商品に500円クーポンを使った場合、2,500円に対して付与するのが原則です。クーポンと重ねて満額付与すると、実質的な二重値引きになります。
- **ショップポイント支払い分** — チケットから交換したショップポイントで支払った金額に、さらにチケットを付けると還元が循環します。除外してください。
- **税込か税抜か** — どちらでも構いませんが、**表示と一致させる**ことが重要です。税込表示のショップで税抜計算にすると、お客様の暗算と合わなくなります。

決めた内容は、チケット制度の規約ページに1行ずつ書き出しておきます。問い合わせが来たときに参照できる形になっていることが大切です。

## 3. 有効期限をどう設計するか

有効期限には2つの型があります。

**固定期限型** — 付与日から12か月など、付与ごとに期限が切れる方式。管理はシンプルですが、お客様は自分のチケットがいつ切れるのか把握しづらくなります。

**最終利用日から延長する型** — 購入やチケット利用があるたびに、保有チケット全体の期限が延びる方式。アクティブなお客様のチケットは実質的に失効しないため、優良顧客を不利にしません。リピート施策としてはこちらが向いています。

期限なしは、会計上の負債が無期限に積み上がることを意味します。小規模ショップでも、期限は設けておくことをおすすめします。目安としては12か月前後が扱いやすい長さです。

## 4. 失効前に知らせるかどうか

期限を設けるなら、失効前の通知をセットで考えます。通知がないまま失効すると、「気づいたら消えていた」という体験になり、チケット制度そのものへの信頼が落ちます。

決めることは2つです。

1. **何日前に知らせるか** — 30日前、7日前など。2段階にすると、1回目を見落としたお客様も拾えます。
2. **何枚以上を対象にするか** — 数枚の失効通知は、かえって煩わしく受け取られます。「送料無料になる」「1品買える」といった、使う動機になる金額から通知すると自然です。

## 5. 誰がチケットを増やせるのか

購入以外にもチケットを付ける経路を用意するかどうかは、初期に決めておきたい論点です。代表的なものは次のとおりです。

- **会員登録時** — 初回購入のハードルを下げます。
- **誕生日** — 年1回、来店理由をつくれます。
- **レビュー投稿** — 商品ページの情報が増えます。
- **友達紹介** — 紹介した側と、された側の両方に付与する形が一般的です。
- **会員クラス** — 累計購入額に応じて還元率を上げ、上位顧客を維持します。
...
<div class="container mx-auto max-w-3xl" data-paginator="{{ paginator.total_pages }}">
  ...
  <header class="mb-10 sm:mb-14">
    ...
    <p class="text-base sm:text-lg text-gray-600">
      カラーミーショップでチケット制度やロイヤルティ施策を運用するための、実践的な記事をお届けします。
    </p>
...
layout: default
title: "タグ一覧"
description: "Illumenza ブログの記事をタグ別にまとめています。チケット制度、ロイヤルティ施策、ECマーケティングなど、関心のあるテーマから記事を探せます。"
---
...
Jekyll's default `slugify` mode keeps Unicode letters, so `チケット制度` should survive as-is. Verify rather than assume — a collapsed slug produces a dangling `#` anchor that silently does nothing:
...
- ミッション機能で「レビュー投稿」にチケットを付ける設定手順
- 会員クラスの区切り方 — 累計購入額のしきい値をどう決めるか
- 友達紹介の付与額を、紹介した側と された側でどう配分するか
- 誕生日ミッションの付与タイミングと、有効期限の合わせ方
- チケット交換（redemption）の単位設計 — 1枚1円以外の選択肢
- チケット失効通知のタイミング設定（30日前・7日前の2段構え）
- チケット制度の規約ページに書くべき項目チェックリスト

## Loyalty / points education

- リピート率とチケット付与率の関係を、粗利から逆算する
- チケット制度とクーポンの使い分け — 新規獲得か、再訪促進か
- 「チケット原資」を会計上どう扱うか（引当の考え方の入口）
- 有効期限あり／なしのメリットとデメリット
- チケット制度をやめる／変更するときの告知の作法

## EC marketing (ColorMe ecosystem)

- カラーミーショップで会員登録率を上げる導線の作り方
- メールマガジンとチケット制度を組み合わせる基本
