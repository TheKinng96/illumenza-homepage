...
braces (`{milesEarned}`), so they need no wrapping.
...
- **Define the thing before using it.** 「1マイル = 1円ではありません」 has to
...
- **Give a number to start from**, then say it is adjustable. 「100円で5マイル
...
- **Convert to something physical.** 「1,250マイル」 means nothing; 「5回買えば
...
<figure>
  <img src="/images/blog/points-expiry-basics/expiry-setting.png"
       alt="マイル有効期限の設定画面。「最終利用日から12か月」を選択した状態。"
